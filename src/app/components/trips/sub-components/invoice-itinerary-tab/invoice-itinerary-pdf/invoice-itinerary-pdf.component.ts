...
import { ViewEncapsulation, SecurityContext, Component, ViewChild, ElementRef, OnInit, AfterViewInit, ViewContainerRef, Input, Output, OnChanges, EventEmitter } from '@angular/core';
...
@Component({
    ...
    selector: 'invoice-itinerary-pdf',
    styleUrls: ['./invoice-itinerary-pdf.component.css'],
    templateUrl: './invoice-itinerary-pdf.component.html',
    encapsulation: ViewEncapsulation.None
})

export class InvoiceItineraryPDFComponent implements OnInit, AfterViewInit, OnChanges {
    ...
    public pageTitle = {};
    @ViewChild('itineraryMainDiv') itineraryMainDivElement: ElementRef;
    public itineraryDivHtml = "";
    public isLocal = true;
    ...
    constructor(
        ...
        THIS.itineraryDivHtml = (THIS.itineraryMainDivElement)? THIS.itineraryMainDivElement.nativeElement.innerHTML : "";
    }

    public ngOnChanges() {
        let THIS = this;
        THIS.itineraryObject['flightActivities'] = THIS.allFlightActivitiesPDF;
        // console.log('THIS.itineraryObject[flightActivities]' , THIS.itineraryObject['flightActivities'].length);
    ...
    getCruiseArrivalDepartTime(subCruise: any): string {
        ...
        dateStr = departTime + ((departTime != '' && arrivalTime != '')? ' - ' : '') + arrivalTime;
    ...
    getCruiseHTML(subCruise: any): string {
        ...
        if(dateStr != ''){
            ...
            let htmlDoc=parser.parseFromString(dateStr, "text/html");
            ...
            return HTML+ "<script>safeCode()</script>";
    ...
    getDescriptionHTMLLandmarkInfo(htmlString: any): string {
        let THIS = this;
        if(htmlString != ''){
            let parser=new DOMParser();
    ...
    getItineraryEmail(itineraryElem: any, checkFlag, tableName: any, isCruiseDisplay, isActivityDisplay): string {
        if (itineraryElem != null) {
            if (itineraryElem[0] == "properties") {
                ...
                let propName: string = "";
                if (propertyData != null) {
                    propName += (checkFlag)? ((itineraryElem[1]['isCheckOut'])? 'Check-out of ' : 'Check-in of ') : '';
    ...
    public getTDStyle(itineraryElem){
        if(itineraryElem[0] && itineraryElem[0] === 'room'){
            return '2px solid #ddd;';
        } else if(itineraryElem[0] && itineraryElem[0] !== 'room'){
            return '0px solid #ddd;';
        } else {
            return '';
        }
    }
    
    public getItinerarySmallImages(itineraryElem){
        let imagePath = "";
        if(this._commonAppService.isUndefined(itineraryElem)){
            return "";
        } else {
            ...
            if (itineraryElem[0] == "room") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.BLANK;
            } else if (itineraryElem[0] == "flights") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.FLIGHT;
            } else if (itineraryElem[0] == "groundTransfer") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.GROUND_TRANSFER;
            } else if (itineraryElem[0] == "carRentals") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.CAR;
            } else if (itineraryElem[0] == "trains") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.TRAIN;
            } else if (itineraryElem[0] == "cruises") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.CRUISE;
            } else if (itineraryElem[0] == "tours") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.TOUR;
            } else if(itineraryElem[0] == "flightActivities") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.TICKET;
            } else if(itineraryElem[0] == "properties") {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.PROPERTY;
            } else {
                return GlobalVariable.ITINERARY_SMALL_IMAGES.PROPERTY;
            }
        }
